...
def analyze_basic_sentiment(text):
    ...
    if not text:
        return "Neutral", "#8a2be2", random.uniform(15, 30)  # Púrpura
    ...
    if bull_score > bear_score:
        return "Positiva", "#00d2ff", random.uniform(20, 45)  # Cyan Neón
    elif bear_score > bull_score:
        return "Negativa", "#ff007f", random.uniform(20, 45)  # Magenta Neón
    else:
        return "Neutral", "#8a2be2", random.uniform(10, 25)  # Púrpura
...
def fetch_rss_news(ticker, num_news):
    ...
    try:
        ...
        for i, item in enumerate(items[:num_news]):
            ...
            sentiment, color, importance = analyze_basic_sentiment(
                title + " " + summary
            )
...
def render_news_network_graph(ticker, num_news=30):
    """
    ...
    """
    news_list = fetch_rss_news(ticker, num_news)
    ...
    for i, news in enumerate(news_list, 1):
        if news["sentiment_color"] == "#00d2ff":
            sentimiento_txt = "Positivo (Alcista)"
        elif news["sentiment_color"] == "#ff007f":
